...
//Console.WriteLine(JsonSerializer.Serialize(customers.Where(x => x.Country==Country.Uzbekistan)));

//Console.WriteLine(JsonSerializer.Serialize(customers.GroupJoin(orders, c => c.Id, o => o.CustomerId, (customers, orders) => new { Customer = customers, HasOrders = orders.Count() > 0 ? true: false})));

//Console.WriteLine(JsonSerializer.Serialize(customers.Where(x => x.FirstName.Equals("John"))));

//Console.WriteLine(JsonSerializer.Serialize(customers.Where(x => x.FirstName == "John" && x.LastName == "Doe")));

//Console.WriteLine(JsonSerializer.Serialize(customers.OrderBy(x => x.FirstName)));
//Console.WriteLine(JsonSerializer.Serialize(customers.OrderBy(x => x.LastName)));
//Console.WriteLine(JsonSerializer.Serialize(customers.OrderBy(x => x.Country)));

//Console.WriteLine(JsonSerializer.Serialize(customers.OrderBy(x => x.Country).ThenBy(x => x.FirstName).ThenByDescending(x => x.LastName)));

for (var i = 0; i < (customers.Count() % 10 == 0 ? customers.Count() / 10 : customers.Count() / 10 + 1); i++)
{
    Console.WriteLine(JsonSerializer.Serialize(customers.Skip(i*10).Take(10)));
    Console.ReadLine();
}