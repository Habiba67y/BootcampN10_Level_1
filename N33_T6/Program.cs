...
Console.WriteLine(JsonSerializer.Serialize(customers.GroupJoin(orders, c => c.Id, o => o.CustomerId, (customers, orders) => new { Customer = customers, Orders = orders.Where(x => x.Amount > 2000)}).Where(x => ((int)x.Customer.Country) == 7)));