...
//1
var customerAndOrders = customers.GroupJoin(orders,
    customer => customer.Id,
    order => order.CustomerId,
    (customer, orderCollection) => new
    {
        Customer = customer,
        Orders = orderCollection
    });
//Console.WriteLine(JsonSerializer.Serialize(customerAndOrders));

//2
var countryOfCustomers = customerAndOrders.GroupBy(c => c.Customer.Country);
//Console.WriteLine(JsonSerializer.Serialize(countryOfCustomers));

//3
var UKCountries = countryOfCustomers.Where(country => country.Key == Country.UK);
//Console.WriteLine(JsonSerializer.Serialize(UKCountries));

//4
var ordersInThisMonth = customerAndOrders.Select(c =>
new
{
    Customer = c.Customer,
    Orders = c.Orders.Where(or => (DateTime.Now - or.OrderDate).TotalDays <= 30)
}).Where(x => x.Orders.Count() > 0);
//Console.WriteLine(JsonSerializer.Serialize(ordersInThisMonth));

//5
var ordersWith5k = customerAndOrders.Select(c =>
new
{
    Customer = c.Customer,
    Orders = c.Orders.Where(or => or.Amount >= 5000)
}).Where(x => x.Orders.Count() > 0);
Console.WriteLine(JsonSerializer.Serialize(ordersWith5k));
